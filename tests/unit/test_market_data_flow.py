...
class TestMarketDataFlow:
    ...
    def test_market_data_manager_connection(self):
        ...
        assert ibkr_client.market_data_manager is not None
        assert ibkr_client.market_data_manager == data_feed.market_data
        assert hasattr(ibkr_client, '_manager_lock')  # Thread safety check
    ...
    def test_thread_safe_market_data_connection(self):
        ...
        # Verify no errors occurred and manager is connected
    ...
    def test_tick_price_error_handling(self):
        ...
        initial_errors = ibkr_client._tick_errors
        ...
        # Verify errors were counted but connection remains functional
        final_errors = ibkr_client._tick_errors
    ...
    def test_health_monitoring_basic(self):
        ...
        # Verify market data flow health structure
        market_flow = health['market_data_flow']
        assert 'manager_connected' in market_flow
        assert 'total_ticks_processed' in market_flow
        assert 'tick_errors' in market_flow
    ...
    def test_health_monitoring_after_ticks(self):
        ...
        # Get initial health
        initial_health = ibkr_client.get_market_data_health()
        assert initial_health['total_ticks_processed'] == 0
        assert initial_health['tick_errors'] == 0
        
        # Process some ticks
        test_ticks = [
            (1, 4, 150.25, None),  # LAST
            (1, 1, 150.20, None),  # BID
            (1, 2, 150.30, None),  # ASK
        ]
    ...
    @patch('src.core.market_data_manager.MarketDataManager.subscribe')
    ...
    def test_market_data_manager_initialization(self):
        ...
        # Mock the order_executor to avoid actual IBKR connection
        with patch.object(ibkr_client, 'is_paper_account', False):
    ...
    def test_concurrent_tick_processing(self):
        ...
        assert health['total_ticks_processed'] == 9
    ...
    def test_error_rate_calculation(self):
        ...
        # Create faulty manager
        class IntermittentFaultyManager:
            def __init__(self):
                self.call_count = 0
        ...
        faulty_manager = IntermittentFaultyManager()
        ibkr_client.set_market_data_manager(faulty_manager)
        
        # Process multiple ticks
        for i in range(10):
            ibkr_client.tickPrice(1, 4, 100.0 + i, None)
        
        # Check error rate calculation
        health = ibkr_client.get_market_data_health()
        assert health['total_ticks_processed'] == 10
        assert health['tick_errors'] > 0
        assert 'error_rate_percent' in health
        assert 0 <= health['error_rate_percent'] <= 100
...
class TestProductionScenarios:
    ...
    @patch('src.core.ibkr_client.IbkrClient.connect')
